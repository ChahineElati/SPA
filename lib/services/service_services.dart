...
void addService(int id, Odoo odoo, String service, double prix) async {
  ...
  odoo.insert('salon.service',
      {'name': service, 'price': prix, 'time_taken': 30.5, 'spa_id': spaId});
}

Future<List<Service>> getServices(int id, Odoo odoo) async {
  var fetchedServices =
      await odoo.query(from: 'salon.service', select: [], where: [
    ['spa_id', '=', id]
  ]);
  List<Service> services = [];
  for (var service in fetchedServices) {
    services.add(Service(service['id'], service['name'], service['price'],
        service['time_taken']));
  }
  return services;
}

Future<List> getServicesByIdsList(Odoo odoo, List servicesIds) async {
  List servicesNames = [];

  List services = await odoo.query(from: 'salon.service', select: []);

  for (var element in services) {
    if (servicesIds.contains(element['id'])) {
